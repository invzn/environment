...
**Canonical spec (source of truth):** `~/.pi/agent/references/philosophy-sdlc/PHILOSOPHY_SDLC.md` + `DESIGN-RECORD.md` + `GLOSSARY.md`. Defer to it for anything unstated.
...
2. **Fast-path check**, derived from the fields themselves: if there is genuinely *no knowledge worth hiding* — no boundary to place; purely mechanical or shallow work — say so plainly and recommend the `implement` prompt instead. Stop unless I override.

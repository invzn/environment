...
Read the design record at: $@ (§1–§2 must be gated; if not, stop).
...
4. **Critique screen** — delegate to `red-flag-auditor` via the subagent tool with §1–§3 only (there is no code to read), screening exactly the five interface-altitude flags: **shallow module, information leakage, overexposure, special-general mixture, pass-through method**. Record `clean` or the finding per flag in §3's critique line — screened, never assumed. For a contract worth extra scrutiny, recommend the `grill-me` skill on it.

...
Read the design record at: $ARGUMENTS (§4 must be filled; if not, stop).

Do NOT modify code yourself — repairs go through the language expert. Bash is read-only for you.

## Steps
1. **Task → `red-flag-auditor`** with the record path and the §4 code/test paths. It is prompted to *find* flaws, not bless the code, against the full red-flag list — weighted toward the four flags LLMs produce characteristically:
...
2. **For each finding, exactly one of:**
   - **Repair** — Task → the language expert with the auditor's proposed repair: pull complexity down, deepen the interface, factor the repetition. Re-run the auditor on the repaired area to confirm.

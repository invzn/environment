...
**Canonical spec (source of truth):** `~/.claude/references/philosophy-sdlc/PHILOSOPHY_SDLC.md` + `DESIGN-RECORD.md`. Defer to it for anything unstated.

Read the design record at: $ARGUMENTS (§5 must be fully verdicted — an empty verdict cell means stage 5 is not done; stop).

Do NOT modify code yourself — refactors go through the language expert. Bash is read-only for you.

## Steps
1. Examine every seam where the change meets the existing system (call sites, wiring, shared types, config).
2. Where it doesn't fit cleanly, **refactor the surrounding design** so the whole stays clean — Task → the language expert, test-first where behaviour moves. **Explicitly reject the "smallest possible change"**: when a minimal bolt-on patch is available (including one the model proposes), record it in §6 as rejected and do the redesign instead. Bolting on at the seam for a minimal diff is the failure this stage guards.

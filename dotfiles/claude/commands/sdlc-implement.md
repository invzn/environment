...
**Canonical spec (source of truth):** `~/.claude/references/philosophy-sdlc/PHILOSOPHY_SDLC.md` + `DESIGN-RECORD.md` + `GLOSSARY.md#pull-complexity-downward`. Defer to it for anything unstated.

Read the design record at: $ARGUMENTS (§1–§3 must be gated; if not, stop).

Do NOT modify code yourself — Task → the matching expert (`go-expert`, `python-expert`, or `language-expert`). Bash is read-only for you (`git diff`, `git log`, `git show`, `git status`).
...
## Steps
1. Task → the expert with: the §3 contract, the §1 budget, the standing instructions, and the project's test conventions.

...
The flag definitions are canonical in `~/.claude/references/philosophy-sdlc/GLOSSARY.md#design-red-flags`. Read them before auditing. Pattern misapplication is in scope: `~/.claude/references/philosophy-sdlc/DESIGN-PATTERNS.md` gives every pattern a **Red flag when** line naming its characteristic shallow form — a Strategy with one strategy, a Factory over one type, a pass-through Facade. Code shaped like one is a finding under the matching flag, citing the catalog entry.

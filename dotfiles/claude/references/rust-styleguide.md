...
- Default to generics (static dispatch): `fn process<B: Backend>(b: &B)` or `fn process(b: &impl Backend)`. Use `dyn Trait` for heterogeneous collections (`Vec<Box<dyn Handler>>`), plugin registries, or to cut compile time/binary size on cold paths.
...
- Run `cargo miri test` in CI for any crate containing `unsafe` — Miri catches UB (use-after-free, invalid aliasing, uninitialized reads) that tests pass over silently
...
- `extern "C"` fns must not panic — wrap bodies in `std::panic::catch_unwind` and translate to an error code

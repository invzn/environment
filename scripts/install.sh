...
install_claude () {
  ...
  # complexity-first SDLC spec (canonical home is repo's philosophy_sdlc/;
  # deployed to a stable runtime path the global /sdlc-* commands read at
  # runtime). Runs after the references reset above so it survives.
  # update_repo.sh only pulls top-level *.md from references/, so this subdir
  # is not mirrored back.
  if [ -d "$repo_dir/philosophy_sdlc" ]; then
    reset_dir "$1/$claude_dir/references/philosophy-sdlc"
    cp -r "$repo_dir/philosophy_sdlc"/. "$1/$claude_dir/references/philosophy-sdlc/"
  fi
  ...
  # shared settings — merged into settings.json rather than copied over it:
  # the live file also holds machine-local keys (hooks, model, theme) that
  # must survive. The repo owns every top-level key shared-settings.json
  # declares (currently permissions and attribution).
  if [ -f "$repo_claude_dir/shared-settings.json" ]; then
    if command -v jq >/dev/null 2>&1; then
      ...
      settings_tmp="$(mktemp)"
      jq -s '.[0] + .[1]' \
        "$settings_file" "$repo_claude_dir/shared-settings.json" > "$settings_tmp"
      mv "$settings_tmp" "$settings_file"
    else
      echo "Warning: jq not found; skipping claude shared settings merge" >&2
    fi
  fi
}
